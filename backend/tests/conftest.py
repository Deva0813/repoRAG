...
async def init_test_db():
    ...
    db: Any = client.get_database(
        "reporag_test", codec_options=CodecOptions(tz_aware=True)
    )
...
@pytest.fixture
def valid_register_admin_payload():
    return {
        "first_name": "Ada",
        "last_name": "Admin",
        "email": "ada.lovelace.admin@example.com",
        "password": "Str0ngPassw0rd!",
        "phone_number": {
            "number": 123457890,
            "country_code": 91,
...
@pytest_asyncio.fixture
async def register_admin_user(client: AsyncClient, valid_register_admin_payload: dict):
    resp = await client.post("/auth/register", json=valid_register_admin_payload)
    user = await User.find_one(User.email == valid_register_admin_payload.get("email"))
    assert user != None
    await user.set({User.role: Role.ADMIN})
    assert user.role == Role.ADMIN
    login_resp = await client.post(
        "/auth/login",
        json={
            "email": valid_register_admin_payload["email"],
            "password": valid_register_admin_payload["password"],
        },
    )
    assert resp.status_code == 201, resp.text
    valid_register_admin_payload["role"] = Role.ADMIN.value
    valid_register_admin_payload["id"] = user.id
    assert login_resp.status_code == 200,login_resp.text
    return valid_register_admin_payload, login_resp
